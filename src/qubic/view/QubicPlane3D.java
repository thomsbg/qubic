 ...
 * Contains low level details about coordinates and contains methods used to
 * help the View3D class with the getSquareAtPoint method.
 * 
...
public class QubicPlane3D {
	 ...
	 * @param h
	 */
	public QubicPlane3D(Point2D p, int w, int h) {
		width = w;
		height = h;
		p1 = p;
	...
	/**
	 * Creates all of the polygons in the plane Creats an Array of all of the
	 * polygons
	 */
	/**
	 * Based on the width, height, and initial point fields, this computes
	 * various points of interest, and from there constructs 25 Point2D objects
	 * representing the various points where grid lines intersect on a board.
	 * With these 25 points, 16 polygons are created representing the 16 squares
	 * in a plane of the game.
	 */
	...
	/**
	 * @return The array of polygons representing the squares of a single plane
	 *         of the game.
	 */
	...
	/**
	 * @param p
	 *            A point that may or not be inside the plane.
	 * @return True is the given point is within the boundaries of the whole
	...
	/**
	 * When asked by the View3D class, it returns the square at the point
	 * clicked. This is called after contains (above), which guarantees that an
	 * appropriate square will be found.
	 * 
	 * @param p
	 *            The point of interest
	 * @param plane
	 *            The integer to put as the z-coordinate of the returned square.
	 * @return The square of interest.
	 */
	public Square getSquareAtPoint(Point2D p, int plane, Board b) {
		for (int i = 0; i < boxes.length; i++) {
			if (boxes[i].contains(p)) {
				int x = 4 - (i % 4);
				int y = i / 4 + 1;
				return b.getSquare(x, y, plane);
			}
		}
	...
	/**
	 * Used internally to determine which polygon corresponds to the given
	 * square. This is used in the highlighting feature.
	 * 
	 * @param s
	 * @return
	 */
	...
	/**
	 * Sets the width of the bottom of one plane of the board. Updated when the
	 * size of the containing panel changes.
	 * 
	 * @param n
	 */
	...
	/**
	 * Sets the initial point from which all the points of interest (and
	 * therefore all the polygons used to draw the boards) are based on.
	 * 
	 * @param p1
	 */
